...
#define key_dbg(bdata, format, arg...)		\
	dev_printk(KERN_DEBUG , &bdata->input->dev , format , ## arg)
#else
#define key_dbg(bdata, format, arg...)	
#endif
...
static ssize_t rk29key_set(struct device *dev, struct device_attribute *attr,
                           const char *buf, size_t count)
{
    struct rk29_keys_platform_data *pdata = dev_get_platdata(dev);
    int i, j, start, end;
    char rk29keyArrary[400];
    struct rk29_keys_Arrary Arrary[] = { { .keyArrary = { "menu" }, }, {
            .keyArrary = { "home" }, },
                                         { .keyArrary = { "esc" }, }, {
                                                 .keyArrary = { "sensor" }, },
                                         { .keyArrary = { "play" }, }, {
...
EXPORT_SYMBOL(rk29_send_power_key);
...
static void keys_long_press_timer(unsigned long _data)
{
    ...
    if (state)
    {
        ...
        {
            if (bdata->long_press_count % (LONG_PRESS_COUNT + ONE_SEC_COUNT)
                    == 0)
            {
...
static void keys_adc_callback(struct adc_client *client, void *client_param,
                              int result)
{
...
static int __devinit keys_probe(struct platform_device *pdev)
{
	...
	for (i = 0; i < pdata->nbuttons; i++) {
		...
		if (button->code_long_press)
			setup_timer(&bdata->timer, keys_long_press_timer, (unsigned long)bdata);else if (button->code)
			setup_timer(
        &bdata->timer, keys_timer, (unsigned long)bdata);
...
if (button->wakeup)
	...
	if (pdata->chn >= 0) {
		setup_timer(&ddata->timer, keys_adc_timer, (unsigned long)ddata);ddata->client = adc_register(pdata->chn, keys_adc_callback, (void *)ddata);
		if (!ddata->client) {
			error = -EINVAL;
			goto fail1;
		}
 ...
 fail2:
	while (--i >= 0) {
		...
		del_timer_sync(
        &ddata->data[i].timer);gpio_free(pdata->buttons[i].gpio);
	}
...
static int __devexit keys_remove(struct platform_device *pdev)
{
	...
	for (i = 0; i < pdata->nbuttons; i++) {
		...
		del_timer_sync(&ddata->data[i].timer);gpio_free(pdata->buttons[i].gpio);
	}
    if(pdata->chn >= 0 && ddata->client);
        adc_unregister(ddata->client);
...
.probe = keys_probe, .remove = __devexit_p(keys_remove), .driver = {
.name = "rk29-keypad", .owner = THIS_MODULE,
#ifdef CONFIG_PM
        .pm = &keys_pm_ops,
#endif
    } };

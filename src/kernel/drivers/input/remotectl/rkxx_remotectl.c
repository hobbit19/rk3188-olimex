...
static int remotectl_keybdNum_lookup(struct rkxx_remotectl_drvdata *ddata)
{
    ...
    for (i = 0;
            i < sizeof(remotectl_button) / sizeof(struct rkxx_remotectl_button);
            i++)
    {
...
static int remotectl_keycode_lookup(struct rkxx_remotectl_drvdata *ddata)
{
    ...
    for (i = 0; i < remotectl_button[ddata->keybdNum].nbuttons; i++)
    {
        ...
        {
            ddata->keycode =
                    remotectl_button[ddata->keybdNum].key_table[i].keyCode;
            return 1;
        }
    }
    return 0;
...
static void remotectl_get_pwr_scanData(struct rkxx_remotectl_drvdata *ddata,
                                       int *pwr_data, int loop)
{
    int i;
    int temp_scanCode;
    int temp_pwr_data;
...
static void remotectl_do_something(unsigned long data)
{
    struct rkxx_remotectl_drvdata *ddata =
            (struct rkxx_remotectl_drvdata *) data;
    ...
    case RMC_SEQUENCE:
    {
        ...
        else if ((TIME_SEQ1_MIN < ddata->period)
                && (ddata->period < TIME_SEQ1_MAX))
        {
            mod_timer(&ddata->timer, jiffies + msecs_to_jiffies(110));
            //printk("2\n");
        }
        else if ((TIME_SEQ2_MIN < ddata->period)
                && (ddata->period < TIME_SEQ2_MAX))
        {
            mod_timer(&ddata->timer, jiffies + msecs_to_jiffies(110));
            //printk("3\n");;
        }
...
{
    struct rkxx_remotectl_drvdata *ddata =
            (struct rkxx_remotectl_drvdata*) _data;
...
{
    struct rkxx_remotectl_drvdata *ddata =
            (struct rkxx_remotectl_drvdata*) dev_id;
    struct timeval ts;
...
{
    printk(KERN_NOTICE "===================================================\n");
    printk(KERN_NOTICE "Rockchip IR remote controller driver. Version 1.0.0\n");
    printk(KERN_NOTICE "===================================================\n");

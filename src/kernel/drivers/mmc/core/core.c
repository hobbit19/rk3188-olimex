...
//the process is default for rockchip SDK. noted by xbw at 2011-11-17
	...
	if (host->host_dev_id == 1)
	{
		...
		{
			printk(KERN_INFO "Initialize SDIO successfully. [%s]\n", mmc_hostname(host));
			return 0;
		}
		else
		{
			if (0xFF != init_ret)
			{
				printk(KERN_WARNING "Initialize SDIO-card unsuccessfully!!! [%s]\n", mmc_hostname(host));
				goto freq_out;
			}
		}
	}
	...
	if (1 != host->rk_sdmmc_emmc_used)
	{
		...
		{
			printk(KERN_INFO "Initialize SD-card successfully. [%s]\n", mmc_hostname(host));
			return 0;
		}
		else
		{
			if (0xFF != init_ret)
			{
				printk(KERN_WARNING "Initialize SD-card unsuccessfully! [%s]\n", mmc_hostname(host));
				goto freq_out;
			}
		}
	}
	...
	{
		printk(KERN_INFO "Initialize MMC-card successfully. [%s]\n", mmc_hostname(host));
	...
	else
	{
		if (0xFF != init_ret)
		{
			printk(KERN_WARNING "Initialize MMC-card unsuccessfully!!! [%s]\n", mmc_hostname(host));
...
#if defined(CONFIG_SDMMC_RK29) || !defined(CONFIG_SDMMC_RK29_OLD)   //Modifyed by xbw at 2011-11-17
	printk(KERN_INFO "mmc_rescan[%s] start...\n", mmc_hostname(host));
#endif

	if (host->ops->get_cd && host->ops->get_cd(host) == 0)
	{
#if defined(CONFIG_SDMMC_RK29) || !defined(CONFIG_SDMMC_RK29_OLD)   //Modifyed by xbw at 2011-11-17
		printk(KERN_WARNING "Find no SDMMC host: %s\n", mmc_hostname(host));
#endif

...
#ifdef DEBUG
#define DBG(x...) printk(KERN_INFO VMAC_NAME ": " x)
#else
#define DBG(x...)
#endif

static unsigned char mac_addr[6] =
{0xfa, 0x45, 0x4f, 0x46, 0x4D, 0x43};
...
int rk29_vmac_sysif_init(void)
{
	...
	if (ret)
	{
		printk(KERN_ERR "%s: Fail to creat class\n",__func__);
		return ret;
	}
	return 0;
}
...
#define vmac_writel(port, value, reg)  writel((value), (port)->regs + reg##_OFFSET)
...
static void write_mac_reg(struct net_device *dev, unsigned char* hwaddr)
{
	...
	if (!is_valid_ether_addr(hwaddr))
	{
		mac_lo = mac_addr[3] << 24 | mac_addr[2] << 16 | mac_addr[1] << 8 | mac_addr[0];
		mac_hi = mac_addr[5] << 8 | mac_addr[4];
	}
	else
	{
...
printk(KERN_ERR "Time out for waiting mdio completion\n");
}
...
static int __devinit vmac_probe(struct platform_device *pdev)
{
	...
	if (ret != 6){
	    printk(KERN_ERR "read mac from Eeprom fail.\n");
	} else {
	    if (is_valid_ether_addr(dev->dev_addr)){
		DBG("MAC from EEPROM: %02X:%02X:%02X:%02X:%02X:%02X\n",
	...
	if (err) {
	    printk(KERN_ERR "RK29_VMAC: Read mac from WiFi fail.\n");
	} else {
	    if (is_valid_ether_addr(dev->dev_addr)) {
		DBG("MAC from WiFi: %02X:%02X:%02X:%02X:%02X:%02X\n",
		    dev->dev_addr[0], dev->dev_addr[1],
		    dev->dev_addr[2], dev->dev_addr[3],
		    dev->dev_addr[4], dev->dev_addr[5] );
		need_mac = 0;
	...
	if (err) {
	    printk(KERN_ERR "RK29_VMAC: read mac from IDB fail.\n");
	    goto err_out_iounmap;
	}
	if (is_valid_ether_addr(dev->dev_addr)) {
	    DBG("MAC from IDB: %02X:%02X:%02X:%02X:%02X:%02X\n",
			dev->dev_addr[0], dev->dev_addr[1],
			dev->dev_addr[2], dev->dev_addr[3],
			dev->dev_addr[4], dev->dev_addr[5]);
		need_mac = 0;
	...
	DBG("MAC from random: %02X:%02X:%02X:%02X:%02X:%02X\n",
	    dev->dev_addr[0], dev->dev_addr[1],
	    dev->dev_addr[2], dev->dev_addr[3],
	    dev->dev_addr[4], dev->dev_addr[5] );
	#else
    ...
    /* overide MAC if kernel parameter has mac address */
    if (strlen(mac_addr_param) == 17)
    {
	DBG("MAC from kcmdl (mac_addr_param): %s\n", mac_addr_param);
	//int i;
	u8 icnt = 0;
	char mb[4] = "\0\0\0";
	...
	for (; p != 0 && icnt < 6; p++) {
	    if (p[0] == ':')
	      continue;
	    memcpy(mb, p, 2);
	    mac_addr[icnt] = (u8) simple_strtoul(mb, NULL, 16);
	    //DBG("%s => %02X\n", mb, mac_addr[icnt]);
	    icnt++;
	    p++;
	}
	...
	if (is_valid_ether_addr(mac_addr)) {
		...
		DBG("MAC from kcmdl: %02X:%02X:%02X:%02X:%02X:%02X\n",
			dev->dev_addr[0], dev->dev_addr[1],
			dev->dev_addr[2], dev->dev_addr[3],
			dev->dev_addr[4], dev->dev_addr[5]);
		need_mac = 0;
    ...
    if (!is_valid_ether_addr(dev->dev_addr) && need_mac) {
		dev_err(&pdev->dev, "Invalid MAC address assigned, aborting.\n");
		err = EINVAL;
		goto err_out_iounmap;
    }

 ...
 *Author:yzq<yzq@rock-chips.com> yxj<yxj@rock-chips.com>
...
extern bool rk29_get_backlight_status(void);
...
if((hdmi_get_hotplug() == HDMI_HPD_ACTIVED) && (hdmi_switch_complete))
{
	...
	if(info != info2)
	{
		...
		{	//the display image of the primary screen is full screen size
...
#ifdef CONFIG_BACKLIGHT_RK29_BL
	/* close backlight for device whic do not support dual display */
